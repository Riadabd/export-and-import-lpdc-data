...
for path in ./lpdc-production-ttl-files/*.ttl; do
    filename=$(basename "$path" .ttl)
    type=$(echo $filename | rev | cut -d '-' -f 1 | rev)

    query=$(cat "$path")
    echo "[INFO] Importing $filename ..."
    isql-v exec="DB.DBA.TTLP_MT(file_to_string_output('/tmp/lpdc-production-ttl-files/$filename.ttl'), '', '$TEMP_GRAPH/$type');"
